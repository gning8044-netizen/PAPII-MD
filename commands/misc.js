...
async function getQuotedOrOwnImageUrl(sock, message) {
    // 1) Image citée (plus haute priorité)
    ...
    // 2) Image dans le message actuel
    ...
    // 3) Avatar du participant mentionné ou cité
    let targetJid;
...
async function handleHeart(sock, chatId, message) {
    try {
        ...
        console.error('Erreur dans misc heart :', error);
        await sock.sendMessage(chatId, { text: '❌ Échec de la création de l’image cœur. Réessaye plus tard.' }, { quoted: message });
    }
}
    ...
    try {
        switch (sub) {
            ...
            case 'its-so-stupid': {
                ...
                if (!dog) {
                    await sock.sendMessage(chatId, { text: 'Usage : .misc its-so-stupid <texte>' }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            case 'namecard': {
                // .misc namecard nom_utilisateur|anniversaire|description(optionnel)
                const joined = rest.join(' ');
                const [username, birthday, description] = joined.split('|').map(s => (s || '').trim());
                if (!username || !birthday) {
                    await sock.sendMessage(chatId, { text: 'Usage : .misc namecard nom_utilisateur|anniversaire|description(optionnel)' }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            case 'oogway2': {
                ...
                if (!quote) {
                    await sock.sendMessage(chatId, { text: `Usage : .misc ${sub} <citation>` }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            case 'tweet': {
                // .misc tweet pseudo_affichage|pseudo|commentaire|thème(optionnel : light/dark)
                const joined = rest.join(' ');
                const [displayname, username, comment, theme] = joined.split('|').map(s => (s || '').trim());
                if (!displayname || !username || !comment) {
                    await sock.sendMessage(chatId, { text: 'Usage : .misc tweet pseudo_affichage|pseudo|commentaire|thème(optionnel light/dark)' }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            case 'youtube-comment': {
                // .misc youtube-comment pseudo|commentaire
                const joined = rest.join(' ');
                const [username, comment] = joined.split('|').map(s => (s || '').trim());
                if (!username || !comment) {
                    await sock.sendMessage(chatId, { text: 'Usage : .misc youtube-comment pseudo|commentaire' }, { quoted: message });
                    return;
                }
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
            ...
            case 'triggered': {
                const avatarUrl = await getQuotedOrOwnImageUrl(sock, message);
                const overlay = sub; // même nom pour le chemin
            ...
            default:
                await sock.sendMessage(chatId, { text: 'Usage : .misc <heart|horny|circle|lgbt|lesbian|nonbinary|pansexual|transgender|lied|lolice|simpcard|tonikawa|its-so-stupid <texte>|namecard u|b|d?|nobitches <texte>|oogway <citation>|oogway2 <citation>|tweet dn|un|c|theme?|youtube-comment un|c>' }, { quoted: message });
                break;
        }
    } catch (error) {
        console.error('Erreur dans la commande misc :', error);
        await sock.sendMessage(chatId, { text: '❌ Échec de la génération de l’image. Vérifie tes paramètres et réessaie.' }, { quoted: message });
    }
}
