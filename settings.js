const settings = {
  packname: '🌹𝐃𝐄𝐕 𝐒𝐇𝐀𝐃𝐎𝐖 𝐓𝐄𝐂𝐇–MD•V3🌹',
  author: 'DEV_SHADOW_TECH',
  botName: "🌹𝐃𝐄𝐕 𝐒𝐇𝐀𝐃𝐎𝐖 𝐓𝐄𝐂𝐇–MD•V3🌹",
  botOwner: 'STIVO-TECH',
  phoneNumber: "221766297256",
  ownerNumber: '221766297256',
  giphyApiKey: 'qnl7ssQChTdPjsKta2Ax2LMaGXz303tq',
  commandMode: "public",
  maxStoreMessages: 20,
  storeWriteInterval: 10000,
  description: "DEV SHADOW TECH MD — Bot WhatsApp en français, rapide, multi-device, créer par DEV_SHADOW_TECH.",
  version: "3.0.1",
  channelLink: "https://whatsapp.com/channel/0029VbBx6Qr3GJOpmxcy6Y1x",
  updateZipUrl: "https://github.com/STIVE-DEVX/ZENTROX-MD-main.zip",
  botPreview: "https://i.postimg.cc/Dz3mbLwc/bot-image-f-Of28PVl.jpg",

  // Carte globale
  adReply: {
    title: "DEV SHADOW TECH BOT MD",
    body: "BY DEV SHADOW TECH",
    thumbnailUrl: 'https://i.postimg.cc/c4KWPt69/chatgpt-image-3-dec-2025-17-48-55-PQCmcx-Vn.jpg',
    sourceUrl: 'https://whatsapp.com/channel/0029VbBx6Qr3GJOpmxcy6Y1x',
    mediaType: 1,
    mediaUrl: 'https://whatsapp.com/channel/0029VbBx6Qr3GJOpmxcy6Y1x',
    renderLargerThumbnail: false
  },

  // Newsletter global (optionnel, utilisable dans toutes les commandes)
  newsletter: {
    newsletterJid: '120363402057857053@newsletter',
    newsletterName: '🌹𝐃𝐄𝐕 𝐒𝐇𝐀𝐃𝐎𝐖 𝐓𝐄𝐂𝐇–MD•V3🌹',
    serverMessageId: -1
  }
};

module.exports = settings;